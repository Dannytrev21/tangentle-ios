"""
Risk Assessor for the Windsurf Planning System.
...
def _check_factor(factor_name: str, step: StepInfo) -> bool:
    """Check if a risk factor is present for a step."""
    checks = {
        "data_migration": lambda s: s.has_data_migration,
        "breaking_change": lambda s: s.is_breaking_change,
        "external_api": lambda s: s.has_external_api,
        "many_file_modifications": lambda s: len(s.files_to_modify) > 5,
        "affects_persistence": lambda s: s.affects_persistence,
        "complex_dependencies": lambda s: len(s.dependencies) > 3,
        "high_complexity": lambda s: s.complexity_estimate == "high",
        "new_files_only": lambda s: len(s.files_to_create) > 0 and len(s.files_to_modify) == 0,
        "documentation_only": lambda s: s.problem_type in ["documentation", "changelog"],
        "low_complexity": lambda s: s.complexity_estimate == "low",
    }
    return checks.get(factor_name, lambda _: False)(step)
...
# Base risk scores for problem types
BASE_RISK_BY_TYPE = {
    # Low risk
    "infrastructure": "low",
    "scaffolding": "low",
...
class RiskAssessor:
    ...
    def _get_base_risk(self, problem_type: str) -> str:
        ...
        return BASE_RISK_BY_TYPE.get(problem_type, "medium")
    ...
    def assess_risk(
        ...
        for name, factor_def in RISK_FACTORS.items():
            present = _check_factor(name, step)

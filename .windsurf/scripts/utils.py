"""
Shared utilities for the Windsurf Planning System.

This module provides common utilities for file operations, output formatting,
and plan management. All paths reference .windsurf/ instead of .claude/.
"""
...
# Default paths - Windsurf equivalent of Claude Code paths
WINDSURF_ROOT = Path(".windsurf")
PLANS_DIR = WINDSURF_ROOT / "plans"
SCRIPTS_DIR = WINDSURF_ROOT / "scripts"
WORKFLOWS_DIR = WINDSURF_ROOT / "workflows"
TEMPLATES_DIR = WINDSURF_ROOT / "templates"
KNOWLEDGE_DIR = WINDSURF_ROOT / "knowledge"
MEMORY_BANK_DIR = WINDSURF_ROOT / "memory-bank"
...
def format_status(status: str) -> str:
    ...
    return f"{indicator} {status.upper()}" if indicator else status.upper()
...
def get_latest_plan(base_dir: Optional[Path] = None) -> Optional[dict]:
    """
    ...
    Returns:
        Plan info dictionary or None if no plans exist
    """
    plans = list_all_plans(base_dir)
    if not plans:
        return None

    # Sort by plan ID (descending) to get the most recent
